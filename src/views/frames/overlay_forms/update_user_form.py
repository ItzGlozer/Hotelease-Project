...
class UpdateUserForm(QWidget):
    ...
    def showForm(self, pre_data):
        self.show()
        if pre_data:
            QMessageBox.information(None, 'Information', 'Fetching real-time user data\nPress OK to continue.')
            userdata = UserRepository.fetchUser(pre_data)
            self._userdata = userdata
            self._firstname_field.setText(userdata['firstname'])
            self._lastname_field.setText(userdata['lastname'])
            self._role_dropdown.setCurrentText(userdata['role'].capitalize())
    ...
    def __validateData(data) -> bool:
        for key, value in data.items():
            if key == 'id': continue # skip id key
    ...
    def _submit(self, controller):
        ...
        if not self.__validateData(data):
            ...
            return
        QMessageBox.information(self, 'Notice', "Updating data\nPress OK to continue")
        controller.updateUserdata(data) # send to database
        self.__closeForm() # close form after submission
        self._overlay.refresh('users') # reload table
        QMessageBox.information(self, 'Notice', "Successfully updated\nTable is refreshed!")